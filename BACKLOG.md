...
给 problems 存一列"目标复杂度"，为了提高LLM打分器的准确性
首次也可考虑"先不评分、默认短间隔(比如 2 天)就进第一次复习"，比让用户自评更干净——但这是优化，不是现在的活。
但是代随不是每道题目都是新知识啊，有些题目知识点就是重复的。但是如果加LLM判断是否是新知识，感觉准确率不高容易误判，假如是老知识别人完美ac了你让他第二天复习，是不是很烦？